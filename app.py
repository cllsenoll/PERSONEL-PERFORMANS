...
def process_excel_data(df):
  ...
  def evaluate_row(row):
    ...
    # AT Zimmet Personel Adı ile Teslim Eden Personel aynı değilse veya boşsa -> DEVİR (Teslim Edilemeyen)
    if t == "" or t != z:
      return "DEVİR", ""

    # AT Zimmet Personel Adı ile Teslim Eden Personel AYNISI -> TESLİM EDİLEN
  ...
  for norm_p, p_df in personnel_groups:
    ...
    # Zimmet Sayısı: AT Zimmet Personel Adı sütununda personelin adının geçtiği toplam satır sayısı
    zimmet_cnt = len(p_df)
...
if st.session_state.active_tab == "Ana Panel":
  ...
  if perf_df is not None and not perf_df.empty:
    ...
    st.markdown(
        "<h3 style='color: #FFB703 !important; margin-bottom: 20px;'>📊 Kurye"
        " Başarı Oranları (%)</h3>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown('<div class="dashboard-card">', unsafe_allow_html=True)
    st.markdown(
        "<h3 style='color: #FFB703 !important; margin-bottom: 20px;'>📲 Teslimat"
        " Kanalları Dağılımı</h3>",
        unsafe_allow_html=True,
    )
    ...
    with chan_sol:
      ...
      st.markdown(
          ...
          ' margin-bottom: 2px;">İMZA</div>',
          unsafe_allow_html=True,
      )
      st.markdown(
          f'<div style="font-size: 13px; color: rgba(255,255,255,0.7);'
          f' margin-bottom: 12px;">Yüzde: %{imza_oran}</div>',
          unsafe_allow_html=True,
      )
    ...
    st.info(
        "💡 Sol menüden **AT ZİMMET İZLEME** dosyanızı yükleyerek ana paneli"
...
# ==========================================
  ...
  if perf_df is not None and not perf_df.empty:
    st.success(
        f"✅ AT ZİMMET İZLEME raporu aktif. Toplam **{len(perf_df)}** kurye"
        " bulundu."
    )
    ...
    st.warning(
        "⚠️ Kurye performans kartlarını görmek için sol menüden **AT ZİMMET"
        " İZLEME** dosyasını yükleyin."
    )
